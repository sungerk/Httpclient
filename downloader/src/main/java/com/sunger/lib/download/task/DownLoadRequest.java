...
import org.sunger.lib.http.download.bean.DownloadEntity;
import org.sunger.lib.http.download.db.Dao;
import org.sunger.lib.http.download.handler.DownloadHandler;
import org.sunger.lib.http.download.utils.FileUtils;
...
public class DownLoadRequest implements Runnable {
	...
	public DownLoadRequest(Context context, String url, String fileName,
			String filePath, final DownloadHandler handler) {
	 ...
	 * @throws Exception
	 */
	private List<DownloadEntity> allocateDownLoadTask(HttpResponse response)
			throws Exception {
		File file = FileUtils.createFile(getFileTempName());
		RandomAccessFile accessFile = new RandomAccessFile(getFileTempName(),
				"rwd");
	 ...
	 * @return
	 */
	private List<DownloadEntity> getDownLoadInTaskInfoFromDB(
	...
	private void sendStartMsg(List<DownloadEntity> downloadInfos) {
		...
		handler.sendStartMessage(getFilePath(), getFileTempName(),
				globalCompelete,
				downloadInfos.get(downloadInfos.size() - 1).endPos);
	}

	private void excuteSubThreads(List<DownloadEntity> downloadInfos) {
		for (DownloadEntity entity : downloadInfos) {
			DownLoadTask task = new DownLoadTask(dao, getFileTempName(),
					entity, handler);
